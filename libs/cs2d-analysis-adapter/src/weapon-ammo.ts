...
export function normalizeWeaponAmmo(raw: Cs2dWeaponAmmo | undefined, weapon: string, tick: number, factRef: string, alive: boolean, samplingVersion?: 2): Partial<ActiveItem> {
  if (!raw || !alive || raw.source !== "SOURCE2_ACTIVE_WEAPON" || raw.phase !== "TICK_END"
    || !Number.isSafeInteger(raw.sampledAtTick) || raw.sampledAtTick < 0 || (samplingVersion === 2 ? raw.version !== 2 || raw.sampledAtTick >= tick : raw.version !== undefined || raw.sampledAtTick !== tick)
  ...
  return { ammo_clip: raw.clip, ammo_evidence: { ...(raw.version === 2 ? { version: 2 as const } : {}), source: raw.source, phase: raw.phase,
    sampled_at_tick: raw.sampledAtTick, weapon_handle: raw.weaponHandle, fact_ref: raw.version === 2 ? `${factRef}-weapon-ammo-end-at-${raw.sampledAtTick}` : `${factRef}-weapon-ammo-end` } };
}
