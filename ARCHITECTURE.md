...
资源诊断使用无身份DecisionResources投影。Host与本地rich DecisionState通过同一计数函数生成可选utilityCount：仅累计明确UTILITY/GRENADE类别（大小写归一），明确WEAPON/KNIFE/BOMB/RIFLE/PISTOL不计，不凭item_id名称猜类别。库存须有完整性元数据且无inventory或其子字段缺失标记；未知类别、缺失/部分库存、非负整数条件不满足或超过诊断边界时不提供确定数，已知空库存可为0。现有诊断边界最多32条库存、每项及utility总数最多64；这是传输约束，不是游戏携带数量断言。
...
可选胜率推理不能因Worker停滞无限阻断基础带看。当前Viewer采用120秒可观察进展空闲窗口：有限且严格增加的下载字节/推理样本分别续期，持续进展不受总时长硬限；无进展则结算失败并沿既有无胜率基础路线，timeline明确UNAVAILABLE，不伪造概率或胜率下降信号。现有WebGPU普通失败转WASM时，仅首次对应失败telemetry重置进度计数域，本身不续期。每次推理独占Worker与timer；成功、失败、取消均一次结算和清理，新推理/换managed Demo/unmount取消旧等待，旧回调不能污染后继任务，取消本身不发布旧分析回退。120秒是运行策略，不是模型性能承诺；RESTORE仍不启动模型。 默认WebGPU读取模型时上报已实际收到的非空字节进度；未知或不能与解码字节比较的总长度保持未知。下载完成仍必须通过完整SHA校验才可创建ORT session，取消/读失败不得以进度完成替代模型成功。


胜率仍在计算时，用户可以主动选择本次使用基础路线。该控制命令只作用于进度事件明确暴露的当前推理身份和所选玩家，不是全局取消；Viewer必须在同一请求的单次完成门中结算，用户跳过沿无胜率回退，换文件/卸载的生命周期取消则不生成旧路线。已完成或已失效请求的跳过命令无副作用。Host入口随当前请求开放并在点击后立即去重，加载/选人/分析结束时失效旧回调；旧版没有推理身份的进度仍可显示，但不能提供这个操作。历史RESTORE仍只播放持久化路线。
...
规范 CandidateSet/hash 仍表示原始提名，准备阶段只施加临时 inference overlay。新binding保存projectionVersion；历史binding缺该字段时按原v1/v2/v3选择规则重建，不能用新投影使旧冻结结果失效。新result另外保存三个原始witness Choice的类别/分布/confidence；model confidence取判断与证据选择的最小值，证据confidence仍单独由实际引用决定。最终 cue 保存 `decisionAssessment`，ReviewPlan 保存 `decision_assessment_run`（版本、模式、调用计数、接受数、拒绝原因、原子输出、usage、延迟与输入 provenance）；旧产物字段可缺省。SessionRecoveryRecord 的严格 FrozenReviewPlan 投影显式接纳该可选 audit，保留三种模式的记录且不重跑评估；最多10次调用、512条候选记录，仍受总记录大小及递归Replay/secret禁入校验约束。准备产物校验失败必须显示错误，不能滞留在提交起点状态。CoachingPackage 在恢复时从冻结 cue 取 artifact，针对原候选重新核对 fingerprint/绑定及适用性。Narrator 只表达通过的判断，decision/outcome 引用与 OutcomeCompletionGate 不变。实验判断不得通过 `verifiedHabitKey` 新增习惯计数；Memory 的 consent、诊断证据、幂等和晋级流程不改。
...
实验 RETURN_AND_FIRE 提名没有基线教学判断，不能抑制同时发生的独立 WIN_RATE_DROP 候选；启用明确射手归属不会因此移除原有教学片段。Adapter 1.6.1 修正该去重行为，旧 1.6.0 及已支持的历史版本继续按保存产物恢复。
...
为避免快速继续造成无必要的后续降级，Host在默认路线实际显示上述baseline时，可登记经当前live、冻结candidate/cue/窗口、Narration身份及完整结果gate验证的内存呈现凭据。后续Controller的observer队列串行消费同一完整identity的凭据，以已有START_CUE且capabilities为空补齐旧教学段，再处理下一段或全场完成。确认必须核对实际cursor/cue/segment、已完成结果、completedCueIds、无工具效果及返回身份，不能只依据COMPLETED状态；补记不调用旧cue的adapter.prepareStart、不mirror旧START、不增加Session反思事件。重置清除凭据并使排队的旧token失效；手动访问、仅历史播放与无可信凭据不自动登记，跨run或真正失败仍走既有fallback。凭据不持久化，不替旧历史补造呈现证明。

已展示教学点的OBSERVE_PRESENTED_CUE回包也必须证明本次事件已被接受：同完整identity、processedEventIds包含该eventId、已呈现绑定匹配cue/segment/index、routeCursor不早于目标且无工具效果，才确认本地cursor并允许mirror。Graph拒绝顺序时返回现有COMPLETED不代表成功；释放未确认请求以允许缺段补齐后重试，不凭历史fallbackReasons判断本次结果。丢ACK后重试可使用保留的已处理事件回执，即使Graph已经前进；回执已超出现有有限窗口时保守不追认。reset前排队或晚回包不得更改新状态，旧promise结束也不得删除同event的新pending请求。

此观察与普通段共用observer串行队列。在完整冻结plan内验证目标cue/segment/index后，若本地最后确认位置到目标之前仅缺普通可观察段，则先用既有段观察按序补齐，完成后才预约并发送目标观察；不能提前预约目标掩盖缺口。若本地记录丢失且间隔含未确认教学段，不替它们制造呈现，仍让Graph依据现有呈现绑定/事件回执接受或拒绝目标。普通段补齐的派发、回包及mirror也受reset token保护。该后台补齐不阻挡基础播放，也不改变Graph顺序门或自动重试网络失败。
...
自由seek/切换回合继续沿既有接管路径使旧意图和manual visit失效；即使尚无Session，显式seek也必须先释放transport暂停再执行。更换Demo、打开历史或发起新的明确教学动作时清除瞬时transport意图；它不持久化为RecoveryBoundary。异步返回默认路线不得覆盖等待期间新的接管或暂停。教练停靠且没有活动播放工具时通用播放按钮禁用，使用既有讲解卡的“再看一遍/继续下一段”及工具busy门。已实际发送的REPLAY_CUE_SLOW或SHOW_GRENADE_TRACE可通过同一按钮暂停/继续演示；Host必须匹配session/run/cue/callId/generation并交由Stage3 Controller发送身份限定的teachingPlayback命令，不发送原始play或USER_TAKEOVER。Viewer保留同一活动工具与播放位置，暂停时阻止延迟启动和完成watch推进，恢复不重启工具。Controller只累计活动播放时间作为ACK期限，暂停墙钟不合成完成或超时。显式seek、取消、失败、超时、iframe load/error和重连使旧控制失效；工具账本持久化await返回后必须重新验证当前调用，晚到结果不得恢复已取消流程。连接变化进入RECOVERY_REQUIRED，经既有账本恢复路径收敛，不直接续播旧工具。静态工具与默认诊断入口不因此获得新的Stage3工具播放能力。
...
Agent Brief的corrections.content完整传递Memory域已经接受的文本，不再截为220字；USER来源和revision仍保留，身份字段仍剥离。含纠正的投影超预算时，先从尾部整条舍弃memories（含advice与限定）再舍activeThreads，必要时清空偏好，最后才舍第二条纠正；保留全部已投影顶层限制。首条完整纠正加必要限制仍超额才EMPTY，不得留下半条纠正；无纠正路径沿用原预算策略；对话异议的500字门与Memory管理纠正1200字门保持，存储内容不迁移。此保证是传递语义完整性，不表示诊断已理解纠正。
...
历史API的SESSION_RECOVERY追加、runtime-head、完整NARRATION_BUNDLE、SESSION_SUMMARY与五类单教学点产物（USER_INTERACTION、CUE_CASE、DIAGNOSTIC_RESULT、TRANSFER_RULE、LEARNING_THREAD）使用每次20秒fetch+JSON共用期限；复用requestJsonWithDeadline并保留非2xx JSON错误code；artifact追加保留原2xx空/非法JSON的void成功语义，runtime-head成功必须返回非空recoveryArtifactId供下一次提交使用。期限先本地结算再abort，迟到响应不继续提交后续head或发布结果。AnalysisBundle及未列出的artifact/历史操作不套此期限。教学请求超时返回TEACHING_SAVE_TIMEOUT，沿既有保存失败处理；产物不完整或interactionDurable=false时禁止本次head镜像，已展示诊断不因保存失败被清空。反思保存失败后既有流程仍可继续诊断，但不能宣称新的恢复点已确认。每请求期限不等于整条保存链20秒。SESSION_RECOVERY实际受smallJsonMaxBytes默认256KiB UTF-8存储限制，head请求上限128000字节；两端还会读取/验证既有AnalysisBundle，20秒是保守客户端等待策略，不是服务端SLA或新增保存证明。

NARRATION_BUNDLE同样属于默认≤256KiB的SQLite小JSON产物，超过上限不能外置；共享artifact路由仍会物化既有分析并验证引用，所以采用现有TEACHING_SAVE_TIMEOUT的20秒fetch+body等待策略。首批讲解保存超时使起点durability失败，沿现有未确认保存反馈与本地激活处理，不继续提交该head；后台讲解超时沿原当前owner错误提示。迟到结果不得继续保存链或自动重试，不重新生成讲解。期限不覆盖大型Analysis/Candidate/Route，也不证明服务端写入被取消。

起点启动只等待实际durabilityCommit（Revision、产物及head），不把历史列表刷新算入该提交。`settlePreparedCoachingStart`在保存结果确认或未确认后反馈相应状态，并调用原`activatePreparedCoachingSession`；失败标记与历史列表刷新是独立、观察错误的收尾，不阻挡本地Session启动。保存未结束仍不能提前激活；本地Recovery持久化、代际/路线门及最新Narration readiness合并保持。激活异常单独报告，不再被当成保存失败重复markFailed。

READY_TO_START同步捕获同一份完整Narration map，供起点恢复记录与persistPreparedReviewStart共同使用；保存阶段不得在await之后改读live map，把后来生成的讲解加入启动必等项。保留当时已有的全部就绪内容，不硬编码只存两项，也不以恢复记录中的Narration summary替代完整bundle。原Revision→Analysis→Candidate→Plan→捕获Narrations→Recovery→head顺序不变，raw Analysis读取仍在Revision完成后的原位置。后续NARRATION_UPDATE通过persistNarrationAfterStart等待起点结算后各自保存一次，不阻挡当前起点激活。两条保存路径捕获Controller及ownershipGeneration，并在异步边界复核准备generation；旧准备不得向新adopt历史继续追加或报告旧错误，失败起点不启动后续head或把未保存讲解标为已落盘。

启动收尾在切代后不再发起后续刷新。历史首屏与分页共用HistoryPageRequests的同步search/cursor/pending归属。保留Sidebar原160ms输入防抖，提交search回调时立即失效旧请求与cursor，不等React effect；捕获旧query的刷新被拒绝。分页只接受当前查询已接受的cursor，pending同步阻止渲染前双击，接受新cursor在React setter之前，已消费游标不能重用。新首屏刷新也能替代同query旧分页。发布结果另经可选代际guard；旧成功/失败不能写新列表、cursor、错误或结束新请求loading，当前分页失败释放pending且保留cursor供明确重试。原追加去重及API20秒期限保持，不新增缓存或自动重试。后台刷新无论成功失败均保留已有保存警告；用户主动刷新沿原清错误/报告读取失败语义。历史list分页摘要（默认30/max50条）和markFailed小PATCH分别使用20秒fetch＋JSON共用期限，复用requestJsonWithDeadline，超时为HISTORY_REQUEST_TIMEOUT；先本地失败结算再best-effort abort，迟到headers/body不再发布，无自动重试。正常query、摘要映射、HTTP错误code及状态PATCH的void ACK语义保持。后台status超时后可以继续一次列表刷新，两次请求合计并非20秒；不是服务端SLA或取消/未落盘保证。此期限不覆盖full detail、大Artifact等未列请求，切代保护仍独立。

创建Review与开始Revision的小元数据POST同样使用HISTORY_REQUEST_TIMEOUT的20秒fetch+JSON共同期限；服务端不在这些请求中解析Demo或调用模型。createForPlayer的reviewPromise或beginRevision的revisionPromise超时后沿原失败路径结算，起点保存未确认仍可激活本地Session；迟到ID不写回Controller或触发后续head。服务端每次生成新UUID，没有创建幂等键，因此不自动重试，不宣称abort撤销已落盘记录。该策略不涵盖后续大Analysis/Candidate/Route上传，也不是整场准备的20秒总保证。

Host本地恢复期间，Runtime自动SELECT_PLAYER的回执只接续待恢复路线，不经PLAYER_SELECTED再创建资料库Review；恢复中的managed导入也不弹“打开最近/创建新复盘”的去重选择，不把一次重新取回Demo当作新建意图。普通managed新玩家选择仍创建一条Review，已有资料库恢复/重新分析沿原分支；本地恢复不会自动迁移为完整SQLite历史，旧本地记录不因该门被删除。创建结果/错误绑定当时Replay对象、player、history open epoch、Controller实例及createForPlayer调用后的ownershipGeneration，正常分析阶段的generation变化不使结果失效；跨选择/同玩家重新选择/adopt后的迟到结果不能更新当前ID或错误。新复盘意图沿既有明确退出恢复流程，不由恢复自动选择推断。
普通managed导入的可选去重提示只属于导入当时尚未选人的意图：Host在导入成功时捕获单调选人序号；收到PLAYER_SELECTED后，旧提示的列表刷新、查询结果、错误与确认后打开均不得发布。该序号不因正常REPLAY_READY而变化，与requestId/history open epoch共同判定当前性；它不改变已有Review、冻结路线或恢复数据。

当前镜像保存失败时不发布新的已确认record/head，Host保留上次已确认记录并说明保存未确认，合法live Agent结果仍可继续。IndexedDB open/transaction沿原1.5秒保护；已写入的IDB缓存或已发HTTP可能在服务器生效，不做回滚/未提交保证。默认桌面激活会话在起点durabilityCommit（含beginRevision）结束后进行；其他持久化Promise或整条队列不新增端到端期限，不新增自动重试。API成功后的正常artifact→head顺序、服务器原单调进度及幂等校验不变。
...
自由描述的目标分类是保守词法投影：显式selectedGoal优先；没有目标关键词才返回未识别并允许修订沿用旧选择。有目标词但仅被否定、含多个不同肯定目标、同一目标正反冲突，或命中有限条件/不确定/他人意图标记时返回UNKNOWN，不能以undefined复活被否定的旧目标。按标点及有限转折/因果连接词分句，只接受唯一未被同目标否定的肯定候选；不支持完整指代、引用、隐含否定、比较或自然语言语义理解。TIME_BELIEF只中性转述时间判断及USER原文，不因出现“时间”就声称存在压力。旧已保存结果不重算；其他claim分类及hinge选择仍有独立关键词规则，目标陈述修复不意味着最终诊断已理解全部否定。

DiagnosticResult/hinge的条件级SUPPORTED、PARTIALLY_SUPPORTED或CONTRADICTED不自动验证任意自由文本USER claim，也不把整包evidenceRefs抄为该claim的supportingRefs/contradictingRefs。现有诊断没有逐条用户命题对应核验时，相关非GOAL陈述标UNVERIFIABLE；未涉及的陈述保持原UNTESTED。GOAL的SUPPORTED仅表示用户自述目标，不表示动作合理或Demo证明了其心理。真实资源数值、人数、条件状态及其引用仍保留在诊断结果，旧保存claim不自动重算；未来逐条核验须提供具体命题与证据对应关系。

信息诊断不从Fact.text中的“没看到”“不存在”“报点不准确”等词直接生成CONTRADICTED：当前Fact/UserClaim没有把用户具体命题、对象、位置、时点与反证对应起来的结构化契约。VERIFY_INFORMATION_ASSUMPTION保留证据引用并返回UNVERIFIABLE，最终INCONCLUSIVE；没有看到不等于不存在，也不能反驳听觉或与事实一致的陈述。说明与条件建议使用通用信息来源/对象/时点措辞，不假定所有用户输入都是声音。旧BELIEF_INCORRECT枚举与已保存结果仍可读，本轮不重判历史；未来恢复肯定/否定判断须有实际可验证主张对应证据，不能靠扩展负向关键词表。

条件分类先生成只读上下文投影：只有命中目标短语且剩余全部属于有限目标表达词的完整分句，才从context检测中排除；原reflection与USER claim原文保持。纯拿信息/补枪/拖时间/执行战术表达不再重复制造敌情、队友、时间或战术信念；窗口、位置、能力、执行问题等独立残词使整句保留。GOAL和指定focus仍能选择原条件，EXECUTE_PLAN直接选择SYNC；TRADE不再扫描所有claim原文，只消费focus、GOAL及TEAMMATE_BELIEF的派生上下文。若旧questionType为TACTICAL_CONTEXT但本轮投影已排除纯目标，残余内容必须含实际战术/语音词才形成战术claim，避免把其他时间/位置背景升为战术。修订重建旧claims也走同一投影，因此保留独立旧陈述而不恢复纯目标伪信念。该有限分句规则不能替代通用语义或实体/时空对应验证；保存结果不重算。
...
该cache也可保存当前已验证的公开回合时钟，复用Host诊断的clock投影与当前Snapshot/决策引用门；不依赖本人资源字段完整。明确回合剩余时间问法仅复述当前诊断已显示且规范ID、约秒值、类型、标签、单位及引用集合全部匹配的measurement。回答必须带决策前最近采样、约秒、非C4倒计时与不能证明等待可行的限制；不从旧保存数字、其他cue、未知或未展示的clock补值。来源更换/清空、未完成结果门或不可信历史沿原问答规则关闭或失效。不扩展为C4时间计算、语义推断、模型请求或时机判断。
