...
启动收尾在切代后不再发起后续刷新。历史首屏刷新按请求序号与可选代际guard发布结果；旧请求不能写新列表/错误或结束新请求的loading。后台刷新无论成功失败均保留已有保存警告；用户主动刷新沿原清错误/报告读取失败语义。该调整解除启动依赖，不为后台status/list请求新增端到端期限或服务器取消保证。
