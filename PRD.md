...
用户在 Apple Silicon macOS 桌面应用中选择本机 Demo 后，产品不是先分析完再交付几条结论，也不是让用户自己浏览数据面板。产品会像真人教练共享屏幕看 Demo 一样，控制复盘节奏，陪用户从比赛开始看到比赛结束。可选 Web 形态复用同一会话契约，但不是桌面首发的前置条件：
...
理想状态下，用户不需要学习复杂数据，也不需要自己知道该看哪一回合。用户只需在桌面应用中选择 Demo，然后跟着教练完成一次复盘。
...
- Apple Silicon macOS 上由 Tauri 启动自包含本地运行时，用户无需预装 Node、Python 或数据库；
- 左侧或中央：本地解析的 2D 战术回放；
...
可选 Web 形态可以复用同一布局与会话契约，但不是默认产品链路。

### 8.2 后续游戏原生 3D 联动形态

后续产品可在不改变教学会话契约的前提下，让主画面使用游戏自身的 3D 回放。产品以桌面侧边窗口或悬浮小窗存在，类似屏幕共享时右侧的教练窗口：
...
产品在用户明确同意后，建立可控的个人教练记忆。原始 Demo 与回放保持本地优先；桌面 MVP 使用 Application Support 下的 SQLite 作为跨 Demo 记忆真相源，并使用稳定的本地主体标识，默认关闭且必须明确 consent。可选 Web 形态可使用 PostgreSQL 与服务端签发的匿名 principal；它不是桌面前置条件。开发环境的纯进程内模式必须明确标注为临时存储，不冒充可恢复的长期记忆。
