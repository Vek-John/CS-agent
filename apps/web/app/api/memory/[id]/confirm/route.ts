...
export async function POST(request: Request, context: Context): Promise<Response> {
  ...
  const parsed = body.value === undefined ? undefined : MemoryConfirmationSchema.safeParse(body.value);
  if (parsed && !parsed.success) return jsonResponse({ accepted: false, reason: "INVALID_CONFIRMATION" }, 400);
  ...
  const record = await runtimeState.service.confirm(requestPrincipal.principal.id, memoryId, parsed?.success ? parsed.data : undefined);
  if (!record) {
    let status = 404;
