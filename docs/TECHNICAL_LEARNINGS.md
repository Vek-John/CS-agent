...
- 决策：offer只绑定本次导入和捕获的选人序号；正常REPLAY_READY不增加序号，真正选人（包括同人再次选择）使旧offer失效。列表刷新、查询结果/错误及确认后打开都检查归属，恢复模式与普通未选人行为沿旧规则。
- 验证：原异步流程小测试4红，修复后10新增/64相关tests及TypeScript/build通过，Host真实回执增加序号。[记录](validation/IMPORT_REVIEW_OFFER.md)。没有完整桌面/系统弹窗实测，没有取消已经发出的服务端查询，也没有改变资料库去重或存储。

## 2026-09-27：导入重试反馈归属

- 问题：主Host新导入仅更新progress，Sidebar同时显示旧error；成功后刷新还默认不检查导入当前性，迟到列表结果可能清除后来请求的错误。
- 决策：有效DEMO_IMPORT_REQUESTED清旧提示；成功刷新显式传现有epoch/requestId判定。取消选择保留原提示，当前失败仍沿原映射显示，过期刷新不发布。复用现有交互/刷新机制，没有新协议或自动重试。
- 验证：49相关tests、最终TypeScript/production build通过，现有refresh测试覆盖过期成功/失败拒绝发布；两行接线由主控集中复查。没有真实桌面交互，不把源码检查与模块测试称为完整Host UI实测。
...
- 验证/限制：55项相关View和时钟回归、TypeScript和production build通过；纯显示文案，无真实Demo/浏览器验证。原时机未知与C4边界保持，不因此追加新判断或道具入口。
...
- 决策：复用opaque cache和规范measurement匹配，原Host clock投影提取为共享helper；公开clock不依赖个人资源完整，答案标明最近采样/约秒/非C4/不证明等待可行。原会话结果门、来源变更失效和零模型调用保持。
- 验证：32新项、149相关tests、TS/build通过，root集中读diff及复验；实际Adapter/Session/诊断→问答及SSR。未真实Demo、浏览器或专业质量评估。[记录](validation/CURRENT_CUE_CLOCK_QUESTIONS.md)。
- 后继：先核实已显示道具种类的明确追问，种类已知不能推成数量已知；不再扩时钟/判决范围。

## 2026-09-26：公开clock与本人资源缺失边界

- 问题：上轮担心Host复用Snapshot新鲜度门会连带丢失公共回合时钟。
- 决策：先走实际Adapter，证实fresh_player_state表示采样帧过旧；本人缺失并不设置它。不新增另一套clock投影或放松资源门。
- 验证：合成WIN_RATE_DROP独立提名、当前frame无本人但clock完整，Host→诊断保持无资源数值、有公开剩余秒measurement、INCONCLUSIVE。新增跨层回归；初始DEATH fixture没有可教学cue，已改为既有独立提名路径。
- 限制：仅合成时钟/模型结果，不代表实际CS-Net推理、真实Demo或UI操作。下一步可接当前已展示时间的明确追问，仍不能回答C4时间或等待是否正确。

## 2026-09-26：已知回合时间是诊断背景，不是时机结论

- 发现：自适应诊断已有具体缺项说明，无需重复问题UI；但时机分支未消费Parser现有公开回合时钟，导致已知与未知混在一起。
- 决策：可选紧凑decisionClock接入Host/严格事件/诊断，全部引用绑定当前DEMO observed决策事实；显示最近采样约秒数，明确非C4、非等待可行性证据，UNVERIFIABLE/INCONCLUSIVE保持。原数值列表直接消费，无新布局。
- 验证：13新增实际Adapter→事件→诊断→Panel SSR，244项不同相关tests、TS/build通过；未知/缺/错身份/时点/边界/引用不补0，独立窄审RELEASE。[记录](validation/DIAGNOSIS_CLOCK_CONTEXT.md)。
- 限制/后继：合成时钟、未浏览器/真实Demo/质量评估；仍缺时机比较器。公共clock目前复用本人Snapshot门，下一项核实本人信息缺失时是否连带丢掉有效公共时间，只用小场景，不放松资源门。
...
- 结果：单读单WASM解析7.458秒；9回合33段、44候选4cue。全跳过与2跳过+2诊断均到Session/Graph COMPLETED、Graph记4cue、摘要写一次，0工具/网络。所有阶段约7.961秒；非用户实际观看耗时。
- 决策：没有生产缺口，不制造修复；增加可复用模块验收工具与匿名证据。没有可验证重复主题时NO_REPEATED_THEME保持，不把完成流程当专业判断正确性的证明。[记录](validation/REAL_GUIDED_LIFECYCLE.md)。
- 限制：没有CS-Net、真实Viewer或SQLite；29相关tests、TS及build结果见记录。下一项使用隔离库验证同类真实产物的保存/关闭重开/恢复及零新分析，重点是实际数据消费而非完整性审计。

## 2026-09-26：已展示cue观察必须排在缺失普通段之后

- 问题：只拒绝错误回包仍会让首次观察等待另一次触发；真实Graph例在缺普通段时首次无法确认。目标提前预约还会隐藏这个缺口。
- 决策：目标校验后进入既有observerTail，先补齐纯普通段间隔再预约目标；普通段与目标及后续段完整串行。重置时补齐回包不能mirror。丢失本地记录且夹有教学段时不编造其呈现，直接由Graph既有回执/绑定判定；初版无差别补齐破坏reset重试，已用该规则修正。
- 验证：新增首尝试1红→绿，加并发三段顺序、reset补齐和非法目标；既有丢ACK/同event重试保持。66相关tests、TS/Web build通过。[证据](validation/PRESENTED_CUE_ORDER.md)。
- 限制：真实模块/合成路线，不是浏览器。该观察队列改进收敛；下一项用授权小Demo验证整场默认路线消费与结束，避免继续围绕状态字段做重复检查。无网络/模型/用户库/新代理。
...
- 决策：每字段正文/引用/限定作为整体，沿既有大小门完整保留或整段明确省略并清refs；省略计入limitationCount，不修改用户讲解正文/能力/旧记录或提高预算。
- 验证：2红→绿；7新测试覆盖实际链、短文、240边界、超引用/尾部第5限定/超长限定及不修改原对象；共41相关测试、TS/Web build通过。[证据](validation/POLICY_SUMMARY_CONDITIONS.md)。
- 限制：长事实是合成合法输入；未测真实模型选择质量、UI或旧历史迁移。该摘要线收敛，下一项核实默认“跳过，直接看分析”的完整用户路径，不为延续工作反复查文本预算。代理与测试进程已释放。
...
## 2026-09-26：目标否定不能被关键词优先级反转

- 问题：真实diagnose初次把“不是拿信息，是保枪”记作拿信息；修订仅否定旧目标时又沿用GET_INFO；TIME_BELIEF把“时间充足/没有时间压力”写成存在压力。目标测试及时间陈述共13红例实际复现。
- 决定：有限分句分类并对肯定候选去重，唯一无同目标否定才采纳；仅否定、多目标、冲突/有限条件词保守UNKNOWN，无关键词才undefined以保留旧值。显式选择始终优先；保留原文/USER来源，时间句中性转述。不增模型/界面/数据协议，不重算历史。
- 验证：真实初次及一次修订、否定换目标、正反冲突、重复同目标、明确选择、他人/条件句、无目标旧值、三种时间陈述；相关6文件119tests、TypeScript/production build通过。不是专家gold或真实Demo质量证明。
- 限制：同一分句的否定会保守抑制其他肯定目标，词法覆盖有限。其他claim/hinge仍扫描“补枪/信息”等词，所以本轮只改善目标和时间陈述的忠实度，不能声称最终诊断已正确理解否定。下一项围绕已知反例核实hinge是否还将被否定的目标当条件，先小例再收窄，不扩通用NLP。


## 2026-09-26：目标表达不能再次冒充条件信念

- 问题：上轮GOAL修复后，“不是补枪，是保枪”仍因claim原文含补枪走TRADE；拿信息/拖时间/执行战术也重复生成不同条件。初次及一次修订共7红例复现。
