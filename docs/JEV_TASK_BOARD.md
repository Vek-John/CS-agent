...
## 2026-09-24：改善观察语义与原子判断（本轮）

基线：干净 `codex/jev-decision-assessment`，`da640a6` 已在origin。目标工具已建立本轮goal；交付本地验证及阶段commit/push，不合并main、不部署或安装。继承并发2/准备6秒预算、取消和历史零重新调用。

| 阶段/验收 | 负责人/文件所有权 | 状态 |
|---|---|---|
| A1 信息损失复现与语义图 | 主控，主builder/契约/任务卡 | 通过：旧损失复现、新默认v4最终HTTP区分合法空间/来源内容 |
| A2 匿名来源/主体/精度投影 | observable_source_audit，独立decision-observation模块/types/tests | 通过：14局部测试；保留源形状/不确定性，粗网格+有界关系，无视线/补枪升级 |
| A3 联合证据题与对照 | decision_core，witness目录与Jev adapter/tests；主控评测 | 完成对照：6题及严格证据一致性；27次live，新Jev明确判断仍0，记录失败 |
| A4 候选/教学/旧恢复 | 主控，主builder/parser/resolver与集成 | 通过：实际准备/消费与旧投影恢复，6秒预算及RETURN保护不变 |
| A5 功能回归、TS/build、文档/commit/push | 主控与独立功能审查 | 1,073tests/TS/Web/Viewer通过，独立功能审查无P1/P2；本阶段随分支提交push |

语义与消费关系：

```mermaid
flowchart LR
  O[候选绑定的合法ObservationClaim] --> B[检查来源、主体精度、时间与共享]
  B --> S[原空间类型 + 粗网格 + 代码计算距离界/方位]
  F[合法公共人数/动作/适用checks] --> P[v4匿名决策包]
  S --> P
  P --> J[3判断 + 3显式联合证据选择]
  J --> V[类别/证据组合/条件/置信度校验]
  V --> D[原Director选择教学价值]
  D --> N[冻结cue与Narrator/本地恢复]
```

必要关系：距离近不证明可补枪，方向不证明LOS，区域/历史位置不升级成精确当前敌人。自身位置粗网格用于避免绝对位置完全消失；网格不是A/B/MID战术区。模型不负责距离或几何计算。单条人数/动作只是联合论证前提，不能要求它独自证明整个风险结论；新题让模型显式选择最小支持集合，不自动粘贴全部白名单。

子任务均继承默认模型/推理，按文件隔离；20分钟阶段检查，30–35分钟上限，只紧凑测试、不解析大Replay、不接触凭证。主控统一最多10次live smoke，若出现有区分度的新信号再进行开发/未用于调题场景分组对照，本轮最多100次请求。引用门和RETURN_AND_FIRE保护不因通过率放松。


本轮实际结果见 [语义阶段验收](validation/JEV_SEMANTIC_ACCEPTANCE.md)：信息损失已修、引用表达更明确，但Jev有效明确判断没有提升；27次后停止本方向，默认实验关闭。冻结代理测试存在家族重合，生成模型相似家族成功不外推专业质量。
